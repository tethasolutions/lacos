...
namespace Lacos.GestioneCommesse.Application.Sync
{
    ...
    public class SyncService : ISyncService
    {
        ...
        public async Task<SyncLocalDbChanges> SyncFromAppToDB_LocalChanges(SyncLocalDbChanges syncLocalDbChanges)
        {
            ...
            try
            {
                await using (var transaction = await dbContext.BeginTransaction())
                {
                    ...
                    syncLocalDbChangesRemote.TicketPictures = await InsertUpdateAllModifiedTicketPictures(syncLocalDbChanges.TicketPictures);
                    ...
                    if (rescheduleActivityId != null)
                    {
                        ...
                        if (alreadyRescheduledActivity == null)
                        {
                            var activity = new Activity()
                            {
                                ...
                                Informations = "Nota da posatore: " + entity.RescheduleNotes,
                                Description = "Nota da posatore: " + entity.RescheduleNotes,
        ...
        private async Task<List<TDto>> InsertUpdateAllModified<TDto, TEntity>(List<TDto> dtoModels) where TEntity : FullAuditedEntity where TDto : SyncBaseDto
        {
            ...
            foreach (var model in dtoModels)
            {
                ...
                { 
                    ...
                    try
                    {
                        ...
                        var syncInterventionNoteDto = model as SyncInterventionNoteDto;
                        var notificationOperators = syncInterventionNoteDto.MessageOperatorIdsList;
                            //    await notificationOperatorRepository.Query()
                            //.AsNoTracking()
                            //.ToListAsync();
                        foreach (var notificationOperator in notificationOperators)
                        {
                            ...
                            notification.OperatorId = (long) notificationOperator;
        ...
        private async Task<List<SyncTicketPictureDto>> InsertUpdateAllModifiedTicketPictures(List<SyncTicketPictureDto> dtoModels)
        {
            var repository = serviceProvider.GetRequiredService<IRepository<TicketPicture>>();
            var ticketRepository = serviceProvider.GetRequiredService<IRepository<Ticket>>();
            var list = new List<SyncTicketPictureDto>();
            ...
            foreach (var model in dtoModels)
            {
                ...
                {
                    if (!model.TicketId.HasValue || model.TicketId <= 0)
                        continue;

                    var ticketExists = await ticketRepository
                        .Query()
                        .AnyAsync(t => t.Id == model.TicketId.Value);

                    if (!ticketExists)
                        continue;

                    var newEntity = model.MapTo<TicketPicture>(mapper);
                    await repository.Insert(newEntity);
                    await dbContext.SaveChanges();
                    list.Add(newEntity.MapTo<SyncTicketPictureDto>(mapper));
                }
            }
